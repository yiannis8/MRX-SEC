...
def clean_issuer(name: str) -> str:
    ...
    words = [w.lower() if i and w.lower() in _SMALL else w for i, w in enumerate(words)]
    return " ".join(words)


_SMALL = {"of", "and", "the", "de", "for"}
...
# --------------------------------------------------------------------------- #
# CSV store (edgar_market.py) — used by the market-comparison app
# --------------------------------------------------------------------------- #

MAREX_RE = re.compile(r"\bMAREX\b", re.I)
...
def _read_store(path: str) -> pd.DataFrame:
    ...
    for c in ("structured", "preliminary"):
        if c in raw:
            raw[c] = raw[c].map(lambda v: str(v).lower() in ("true", "1"))
    return raw


def load_market(store_path: str, structured_only: bool = True) -> pd.DataFrame:
    """Read the market store, merge preliminary/final per ISIN, normalise
    dtypes and names, and flag Marex. Same output shape as load_notes()."""
    rows: list[dict] = []
    ...
    df["preliminary"] = df["preliminary"].fillna(False).astype(bool)
    df["structured"] = df["structured"].fillna(False).astype(bool)
    df["issuer_raw"] = df["issuer"].fillna("Unknown")
    df["issuer"] = df["issuer_raw"].map(clean_issuer)
    df["is_marex"] = df["issuer_raw"].str.contains(MAREX_RE)
    if structured_only:
        df = df[df["structured"]]
    return df.reset_index(drop=True)


def store_status(store_path: str) -> dict:
    if not os.path.exists(store_path):
        return {"exists": False}
    raw = pd.read_csv(store_path, usecols=["filed", "accession", "issuer"], dtype=str)
    return {"exists": True, "rows": len(raw), "issuers": raw["issuer"].nunique(),
            "first": raw["filed"].min(), "last": raw["filed"].max(),
